...
void TestQ1_add2(CuTest *tc) {
	...
	int n = 3;
	double input1[3] = {2.60, 4.18, 6.05};
	double input2[3] = {1.30, 2.20, 3.80};
	double actual[3];
	double expected [3] = {3.90, 6.38, 9.85};
	add_vectors(input1,input2,actual,n);
	int i;
	for (i=0; i<n; i++)
		CuAssertDblEquals(tc, expected[i], actual[i],0.009);
...
void TestQ2_true_fullnegative(CuTest *tc) {

	int input[3][3] = {{-8, -2, -5}, {-6, -8, -1}, {-3, -5, -9}};
	int actual = is_diag_dom(input);
	int expected = 1;
	CuAssertIntEquals(tc, expected, actual);
}
...
void TestQ6_combined2(CuTest *tc) {
	int n=8;
	int input[]={0,0,25,0,-5,0,45,0};
	int actual_output_val[8]={0};
	int actual_output_pos[8]={0};
	int actual_output[8]={0};
	efficient(input,actual_output_val,actual_output_pos,n);
	reconstruct(actual_output,8,actual_output_val,actual_output_pos,3);
	int expected_output[8]= {0,0,25,0,-5,0,45,0};
...
void TestQ7_equalSizeInputs2(CuTest *tc) {
	...
	// HARDCODED testcases
   		int input_val1[] = {11,22,33};
	int input_pos1[] = {2,3,7};
	int input_val2[] = {44,55,66};
	int input_pos2[] = {3,5,6};
	...
	int expected_output_val[6]={11,  66,  55,  66,  33,0};
	int expected_output_pos[6]={2,   3,   5,   6,   7,0};
...
CuSuite* Lab2GetSuite() {
	...
	SUITE_ADD_TEST(suite, TestQ2_true_fullnegative);
	...
	SUITE_ADD_TEST(suite, TestQ4_Aa);
